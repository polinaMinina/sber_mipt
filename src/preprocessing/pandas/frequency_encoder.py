...
class CustomFrequencyEncoder(ColTransformerPandasMixin, ColCategoryTransformer):
    ...
    def fit(self, x: pd.DataFrame):
        super().fit(x)
        pd_col = x[self.col_name_original].astype('category')
    ...
    def transform(self, x: pd.DataFrame):
        ...
        x = self.attach_column(
            x, 
            optimize(x[self.col_name_original].astype('category').map(self.mapping).astype('float').fillna(self.other_values_code).astype('int64')).rename(self.col_name_target))
        x = super().transform(x)
        return x
